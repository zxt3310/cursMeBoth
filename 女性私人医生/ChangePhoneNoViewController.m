...
- (void)codeBtnAction{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *post = [NSString stringWithFormat:@"source=apple&version=3.3&appid=4&switchType=1&os=ios&imei=%@&deviceid=%@&username=%@&mobileTel=%@&userid=%ld",[CureMeUtils defaultCureMeUtil].UDID,[CureMeUtils defaultCureMeUtil].uniID,[CureMeUtils defaultCureMeUtil].userName,PhoneTF.text,[CureMeUtils defaultCureMeUtil].userID];
...
- (void)sendBtnClickAction{
    ...
    NSString *post = [NSString stringWithFormat:@"source=apple&os=ios&appid=4&version=3.3&mobile=%@&mobileverify=%@&deviceid=%@&userid=%ld&username=%@&imei=%@",PhoneTF.text,codeTF.text,[CureMeUtils defaultCureMeUtil].uniID,[CureMeUtils defaultCureMeUtil].userID,[CureMeUtils defaultCureMeUtil].userName,[CureMeUtils defaultCureMeUtil].UDID];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    NSData *response = sendFullRequest(urlStr, post, nil, NO, NO);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (!response) {
