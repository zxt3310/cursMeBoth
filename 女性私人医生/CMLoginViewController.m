...
- (void)recieveAuthResponse:(BOOL)isSucced code:(NSString *)code{
    ...
    NSString *post = [NSString stringWithFormat:@"source=apple&imei=%@&addrdetail=%@&ersion=%@&appid=4&deviceid=%@&os=ios",[CureMeUtils defaultCureMeUtil].UDID,[CureMeUtils defaultCureMeUtil].encodedLocateInfo,[CureMeUtils defaultCureMeUtil].appVersion,[CureMeUtils defaultCureMeUtil].uniID];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
