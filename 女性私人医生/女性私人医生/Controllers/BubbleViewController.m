...
- (void)showHospitalDetailPage
{
    ...
    [webVC setStrURL:[NSString stringWithFormat:@"http://new.medapp.ranknowcn.com/h5_new/server/app.php?type=doctorinfo&hid=%ld&did=%ld&hname=%@&appid=4",(long)hospID,(long)_doctor.doctorID,_doctor.hospitalName]];
...
{
    if (![CureMeUtils defaultCureMeUtil].hasLogin) {
        NSString *post = [NSString stringWithFormat:@"action=createuserdata&deviceid=%@&appid=4&addrdetail=%@&token=%@", [CureMeUtils defaultCureMeUtil].uniID, [CureMeUtils defaultCureMeUtil].encodedLocateInfo, nil];
        NSData *response = sendRequest(@"m.php", post);
        NSString *strResp = [[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding];
